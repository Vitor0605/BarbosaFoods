...
import {
  ...
  signOut, onAuthStateChanged, browserSessionPersistence, inMemoryPersistence,
  setPersistence, getIdTokenResult
} from "https://www.gstatic.com/firebasejs/12.15.0/firebase-auth.js";
import {
  getFirestore, collection, doc, updateDoc, deleteDoc,
  onSnapshot, query, where, getDoc, setDoc, getDocs, runTransaction, writeBatch,
  serverTimestamp
...
const MAX_SODAS       = 3;
const MAX_SWEETS      = 10;
const MAX_ORDERS_PER_DAY  = 3;       // limite diário de tentativas, contando cancelados
const ALLOWED_SODAS   = new Set(['Guaraná Zero', 'Coca Zero']);
const ALLOWED_JUICES  = new Set(['Suco de Laranja', 'Suco de Morango', 'Suco de Abacaxi']);
...
let cleanupOldCanceledRunning = false;
...
function getOrderDateText(order) {
  return fmtDate(order && order.timestamp) || String(order?.dateStr || '');
}

function getOrderTimeText(order) {
  return fmtTime(order && order.timestamp) || String(order?.timeStr || '');
}
...
function sortOrdersDesc(orders) {
  return [...orders].sort((a, b) => {
    const bDate = toDate(b && (b.timestamp || b.createdAt));
    const aDate = toDate(a && (a.timestamp || a.createdAt));
...
function countValidOrders(orders) {
  // Pedidos cancelados não contam em nenhum limite total; só vale o limite diário.
...
function analyzeOrderPermission(orders) {
  ...
  if (todayOrders.length >= MAX_ORDERS_PER_DAY) {
    return {
      podePedir: false,
      motivo: `Você já fez ${MAX_ORDERS_PER_DAY} pedidos hoje. Esse limite diário conta pedidos pagos, pendentes e cancelados. Amanhã o limite libera novamente.`,
      pedidos: userOrders,
      validOrders,
      todayOrders,
      openOrder: null,
...
function getProfileChangeStatus() {
  const today = getTodayKey();
  const lastDate = toDate(currentProfile && currentProfile.lastProfileEditAt);
  const last = lastDate ? getTodayKey(lastDate) : '';
...
async function saveProfileInfo(nome, turma, options = {}) {
  ...
  await setDoc(ref, {
    ...
    lastProfileEditAt: serverTimestamp(),
    updatedAt: serverTimestamp()
  }, { merge: true });
...
function getProfileFormValues() {
  ...
  const select = document.getElementById('profile-turma-select');
  return {
    nome: normalizeName(input ? input.value : ''),
    turma: select ? String(select.value || '').trim() : ''
  };
}

function profileFormHasChanges() {
  const { nome, turma } = getProfileFormValues();
  return nome !== getProfileName() || turma !== getProfileTurma();
}

function updateProfileSaveState() {
  const btn = document.getElementById('profile-save-btn');
  const status = document.getElementById('profile-save-status');
  const hasChanges = profileFormHasChanges();
  const locked = getProfileChangeStatus().locked;

  if (btn) {
    btn.disabled = !hasChanges || locked;
    btn.textContent = locked && hasChanges ? 'Liberado amanhã' : 'Salvar alterações';
  }

  if (status && !hasChanges) {
    status.textContent = 'Altere o nome ou a turma para liberar o botão de salvar.';
...
async function saveProfileFromModal() {
  ...
  if (!profileFormHasChanges()) {
    if (status) { status.textContent = 'Nada foi alterado. Mude o nome ou a turma antes de salvar.'; status.className = 'profile-status'; }
    updateProfileSaveState();
    return;
  }
  ...
  try {
    ...
    if (status) { status.textContent = 'Perfil atualizado. Pedidos já registrados permanecem como histórico imutável.'; status.className = 'profile-status ok'; }
    onNomeInput();
    updateProfileSaveState();
  } catch (err) {
...
function renderOrderCards(listEl, pedidos) {
  if (!listEl) return;
  if (!pedidos.length) {
    listEl.innerHTML = '<div class="empty-state small">Você ainda não fez nenhum pedido.</div>';
    return;
  }
  ...
  listEl.innerHTML = sortOrdersDesc(pedidos).map(o => {
    ...
    const cancelBtn = canCancel ? `<button class="profile-cancel-btn" type="button" data-action="open-cancel" data-order-id="${fid}">Cancelar pedido</button>` : '';
    return `
      <div class="profile-order-card ${isOrderCanceled(o) ? 'canceled' : ''}">
        ...
        <div class="profile-order-meta">${escapeHtml(getOrderDateText(o))} · ${escapeHtml(getOrderTimeText(o))} · ${escapeHtml(o.turma || '')}</div>
...
function renderProfileOrders() {
  ...
  const meusPedidosHoje = getTodayOrders(allOrders.filter(o => o.userUid === currentUser.uid));
  renderOrderCards(list, meusPedidosHoje);
}
...
function renderStudentHome() {
  ...
  const meusPedidosHoje = status.todayOrders ? status.todayOrders : getTodayOrders(meusPedidos);
  const todayCount = meusPedidosHoje.length;
  ...
  if (countBox) {
    countBox.textContent = `${todayCount}/${MAX_ORDERS_PER_DAY} pedidos feitos hoje`;
  }
  ...
  if (alertBox) {
    if (status.podePedir) {
      ...
      alertBox.textContent = todayCount ? 'Você ainda pode fazer pedido hoje.' : 'Você ainda não fez pedidos hoje. Pode começar agora.';
    } else {
      alertBox.style.display = 'block';
  ...
  renderOrderCards(ordersList, meusPedidosHoje);

  if (btn) {
    btn.disabled = !status.podePedir;
    btn.textContent = status.podePedir ? 'Fazer novo pedido →' : 'Novo pedido bloqueado';
  }
}
...
function openCancelModal(fid) {
  ...
  if (text) text.textContent = 'Esse pedido será marcado como cancelado. Ele continua contando no limite diário de 3 pedidos, mas amanhã o limite libera novamente.';
  if (preview) {
    preview.innerHTML = `
      <strong>${fmtBRL(order.total || 0)}</strong>
      <span>${escapeHtml(getOrderDateText(order))} · ${escapeHtml(getOrderTimeText(order))}</span>
...
async function confirmCancelOrder() {
  ...
  try {
    ...
    // cancelMyOrder mantém o modal aberto e apresenta a mensagem segura ao usuário.
  } finally {
...
async function cancelMyOrder(fid, options = {}) {
  ...
  if (!isOrderOpen(order)) {
    const text = document.getElementById('cancel-modal-text');
    if (text) text.textContent = 'Este pedido não está mais pendente e não pode ser cancelado.';
  ...
  try {
    ...
    await runTransaction(db, async transaction => {
      ...
      if (!['aguardando_pix', 'pendente_dinheiro'].includes(pedido.status) || pedido.cancelado === true) {
        throw new Error('Somente pedidos pendentes podem ser cancelados.');
      }
...
async function routeUser(user) {
  ...
  if (!user.emailVerified) {
    isAdmin = false;
    await signOut(auth);
    showAuthMessage('Use uma conta Google com e-mail verificado.', 'error');
    return;
  }
  ...
  try {
    ...
    tokenIsAdmin = tokenResult.claims.admin === true;
  } catch (err) {
    console.warn('Não foi possível ler as permissões adicionais da conta:', err);
  }
  isAdmin = tokenIsAdmin || ADMIN_EMAILS.map(e => e.toLowerCase()).includes(email);
  ...
  wrap.innerHTML = TURMAS_GRUPOS.map(g => `
      <div class="turma-grupo">
        ...
        <div class="turma-chips">
          ${g.turmas.map(t =>
          `<button type="button" class="turma-chip" data-action="select-class" data-turma="${escapeHtml(t)}">${escapeHtml(t)}</button>`
        ).join('')}
      </div>
    </div>
  `).join('');
...
function selectBread(type) {
  if (!Object.prototype.hasOwnProperty.call(BREAD_PRICES, type)) return;
...
function selectJuice(el) {
  ...
  if (!ALLOWED_JUICES.has(el.dataset.juice || '')) return;
...
function addSoda(flavor) {
  if (!ALLOWED_SODAS.has(flavor)) return;
...
function removeSoda(index) {
  if (!Number.isInteger(index) || index < 0 || index >= sodaCart.length) return;
...
function renderSodaCups() {
  ...
  list.innerHTML = sodaCart.map((flavor, i) => `
    <div class="soda-cup-chip">
      ...
      <span class="soda-chip-name">${escapeHtml(flavor)}</span>
      <button type="button" data-action="remove-soda" data-index="${i}" class="soda-cup-remove" title="Remover" aria-label="Remover ${escapeHtml(flavor)}">
  ...
  if (list) {
    list.innerHTML = entries.map(item => `
      <div class="sweet-chip">
        ...
        <button type="button" class="sweet-chip-remove" data-action="remove-sweet" data-sweet="${escapeHtml(item.id)}" title="Remover 1 ${escapeHtml(item.name)}" aria-label="Remover 1 ${escapeHtml(item.name)}">
...
function selectPayment(type) {
  if (!['pix', 'dinheiro'].includes(type)) return;
...
async function submitOrder() {
  ...
  try {
    ...
    await runTransaction(db, async transaction => {
      ...
      const dailyOrderAt = toDate(userData.dailyOrderAt);
      const isCurrentSchoolDay = dailyOrderAt && getTodayKey(dailyOrderAt) === getTodayKey(now);
      const dailyOrderCount = isCurrentSchoolDay ? Number(userData.dailyOrderCount || 0) : 0;
      ...
      if (dailyOrderCount >= MAX_ORDERS_PER_DAY) throw new Error('Limite diário de pedidos atingido.');
      if (userData.nome !== nome || userData.turma !== turma) {
        throw new Error('Seu perfil mudou em outro dispositivo. Recarregue a página antes de pedir.');
      }
      ...
      transaction.set(userRef, {
        ...
        lastOrderCreatedAt: serverTimestamp(),
        dailyOrderCount: dailyOrderCount + 1,
        dailyOrderAt: serverTimestamp(),
        updatedAt: serverTimestamp()
      }, { merge: true });
    });
...
function isCanceledOrderOlderThanOneDay(order) {
  if (!isOrderCanceled(order)) return false;
  const base = order.canceledAt || order.timestamp || order.createdAt;
...
async function cleanupOldCanceledOrders() {
  if (!isAdmin) return;
  if (cleanupOldCanceledRunning) return;
  const oldCanceled = allOrders.filter(isCanceledOrderOlderThanOneDay);
  if (!oldCanceled.length) return;

  cleanupOldCanceledRunning = true;
  try {
    await Promise.all(oldCanceled.map(o => deleteDoc(doc(db, 'pedidos', o.firestoreId))));
  } catch (err) {
    console.warn('Não foi possível apagar pedidos cancelados antigos agora:', err);
  } finally {
    cleanupOldCanceledRunning = false;
  }
}
...
function subscribeToAdminOrders() {
  ...
  // Sem orderBy para manter compatibilidade durante a migração dos timestamps antigos (string)
  // para timestamps de servidor. A ordenação segura é feita localmente por sortOrdersDesc.
...
function renderAdmin() {
  ...
  const arrecadado = allOrders.filter(o => o.status === 'pago').reduce((s, o) => s + (Number(o.total) || 0), 0);
  const aReceber   = allOrders.filter(o => isOrderOpen(o) && !o.oculto).reduce((s, o) => s + (Number(o.total) || 0), 0);
  ...
  const turmasHtml = Object.entries(turmaCounts)
    ...
    .map(([t, c]) => `<button type="button" class="turma-badge ${currentTurmaFilter === t ? 'active' : ''}" data-action="filter-class" data-value="${escapeHtml(t)}">${escapeHtml(t)}<span>${c} pedido${c !== 1 ? 's' : ''}</span></button>`)
    .join('');
...
function renderOrders() {
  ...
  const sorted = sortOrdersDesc(filtered);
  ...
  list.innerHTML = sorted.map(o => {
    ...
    const confirmBtn = (!isOculto && isOrderOpen(o))
      ? `<button class="btn-confirm" data-action="confirm-payment" data-order-id="${fid}">✓ Pago</button>` : '';
    const ocultarBtn = !isOculto
      ? `<button class="btn-action btn-hide" data-action="toggle-hidden" data-order-id="${fid}">👁 Ocultar</button>`
      : `<button class="btn-action btn-show" data-action="toggle-hidden" data-order-id="${fid}">↩ Mostrar</button>`;
        ...
        <div class="order-header">
          <div>
            ...
            <div class="order-meta">
              ...
              <span class="order-date">📅 ${escapeHtml(getOrderDateText(o))}</span>
              <span class="order-time">🕐 ${escapeHtml(getOrderTimeText(o))}</span>
            </div>
          </div>
          <div>${badge}</div>
        </div>
...
async function confirmPayment(fid) {
  ...
  try {
    const pedidoRef = doc(db, 'pedidos', fid);
    const localOrder = allOrders.find(order => order.firestoreId === fid);
    if (!localOrder || !isOrderOpen(localOrder)) throw new Error('Este pedido não está mais pendente.');
    if (!confirm(`Confirmar o pagamento de ${localOrder.name || 'este aluno'} no valor de ${fmtBRL(localOrder.total)}? Essa ação não pode ser desfeita.`)) return;
    ...
    await runTransaction(db, async transaction => {
      ...
      const pedido = pedidoSnap.data();
      if (!['aguardando_pix', 'pendente_dinheiro'].includes(pedido.status) || pedido.cancelado === true) {
        throw new Error('Somente pedidos pendentes podem ser marcados como pagos.');
      }
      ...
      if (userRef && userSnap && userSnap.exists() && userSnap.data().activeOrderId === fid) {
        transaction.set(userRef, {
          activeOrderId: null,
          lastPaidOrderId: fid,
  ...
  try {
    ...
    grouped.forEach((orders, uid) => {
      ...
      batch.set(doc(db, 'usuarios', uid), {
        ...
        dailyOrderCount: todayOrders.length,
        dailyOrderAt: serverTimestamp(),
...
const REDIRECT_LOGIN_PENDING_KEY = 'bf_google_redirect_pending';

function markRedirectLoginPending() {
  try { sessionStorage.setItem(REDIRECT_LOGIN_PENDING_KEY, '1'); }
  catch (_) {}
}

function clearRedirectLoginPending() {
  try { sessionStorage.removeItem(REDIRECT_LOGIN_PENDING_KEY); }
  catch (_) {}
}

function consumeRedirectLoginPending() {
  try {
    const pending = sessionStorage.getItem(REDIRECT_LOGIN_PENDING_KEY) === '1';
    sessionStorage.removeItem(REDIRECT_LOGIN_PENDING_KEY);
    return pending;
  } catch (_) {
    return false;
  }
...
function explainAuthError(err) {
  ...
  const host = window.location.hostname || 'este domínio';
  if (code === 'auth/unauthorized-domain') return `Este domínio (${host}) ainda não está autorizado no Firebase Authentication. Adicione ele em Authentication > Settings > Authorized domains.`;
  if (code === 'auth/internal-error') return 'O Firebase não conseguiu concluir o login do Google neste navegador. Recarregue e tente de novo; se continuar, confira os domínios autorizados do Firebase/Google e tente pelo Chrome ou Safari normal.';
  if (code === 'auth/popup-blocked') return 'O navegador bloqueou a janela do Google. Permita pop-ups para este site ou tente novamente pelo Chrome/Safari normal.';
  if (code === 'auth/popup-closed-by-user' || code === 'auth/cancelled-popup-request') return 'Login cancelado antes de concluir.';
  if (code === 'auth/operation-not-allowed') return 'O login com Google não está ativado no Firebase Authentication.';
  if (code === 'auth/admin-restricted-operation') return 'Esta conta Google pode estar bloqueada pela organização para entrar em apps externos.';
  if (code === 'auth/account-exists-with-different-credential') return 'Este e-mail já existe com outro método de login.';
  if (code === 'auth/user-disabled') return 'Esta conta foi desativada no Firebase Authentication.';
  if (code === 'auth/network-request-failed') return 'Falha de rede durante o login. Recarregue a página e tente novamente.';
  if (code === 'auth/web-storage-unsupported') return 'O navegador bloqueou cookies/armazenamento necessários para manter o login.';
  if (code === 'auth/redirect-cancelled-by-user') return 'O login por redirecionamento foi cancelado.';
  if (code === 'auth/redirect-operation-pending') return 'Já existe uma tentativa de login em andamento. Aguarde ou recarregue a página.';
  return (err && err.message) ? err.message : 'Não foi possível concluir o login.';
}

async function ensureAuthPersistence() {
  // Em computadores compartilhados, a sessão termina quando o navegador é fechado.
  const options = [browserSessionPersistence, inMemoryPersistence];
  let lastError = null;
  for (const persistence of options) {
    try {
      ...
      lastError = err;
      console.warn('Persistência de login indisponível, tentando outra opção:', err);
    }
  }
...
if (consumeRedirectLoginPending()) {
  getRedirectResult(auth)
    .then(result => {
      if (result && result.user) showAuthMessage('Login concluído. Carregando seus dados...', 'ok');
    })
    .catch(err => {
      console.error('Erro no retorno do login:', err);
      showAuthMessage(`${explainAuthError(err)} Código: ${err.code || 'sem-codigo'}`, 'error');
    })
    .finally(clearRedirectLoginPending);
}

async function loginWithGoogle() {
  const btn = document.getElementById('btn-google-login');
  showAuthMessage('Abrindo login do Google...', 'info');

  try {
    setButtonLoading(btn, true, 'Abrindo Google...');
    await ensureAuthPersistence();

    // Em Vercel, o fluxo signInWithRedirect pode falhar em mobile por bloqueio de armazenamento
    // de terceiros. Por isso o fluxo principal agora é popup em todos os navegadores.
    // No Chrome Android ele abre como uma janela/aba do Google e volta para o app mantendo a sessão.
    await signInWithPopup(auth, provider);
    showAuthMessage('Login concluído. Carregando seus dados...', 'ok');
  } catch (err) {
    const code = err && err.code ? err.code : '';
    ...
    if (fallbackCodes.has(code)) {
      try {
        showAuthMessage('Popup bloqueado. Tentando login em tela inteira...', 'info');
        markRedirectLoginPending();
        await signInWithRedirect(auth, provider);
        return;
      } catch (redirectErr) {
        clearRedirectLoginPending();
        console.error('Erro no fallback de redirecionamento:', redirectErr);
        showAuthMessage(`${explainAuthError(redirectErr)} Código: ${redirectErr.code || 'sem-codigo'}`, 'error');
        return;
      }
    }

    console.error('Erro ao fazer login:', err);
    showAuthMessage(`${explainAuthError(err)} Código: ${code || 'sem-codigo'}`, 'error');
  } finally {
...
async function confirmLogout() {
  ...
  try {
    ...
    if (btn) { btn.disabled = false; btn.textContent = 'Sim, sair'; }
  }
}
...
async function handleActionClick(event) {
  ...
  switch (action) {
    ...
    case 'login-google': await loginWithGoogle(); break;
    case 'open-profile': openProfile(); break;
    case 'close-profile': closeProfile(); break;
    case 'save-profile': await saveProfileFromModal(); break;
    case 'logout': logout(); break;
    case 'close-logout': closeLogoutModal(); break;
    case 'confirm-logout': await confirmLogout(); break;
    case 'start-order': startNewOrder(); break;
    case 'continue-identification': await continuar(); break;
    case 'back-identification': voltarIdentificacao(); break;
    case 'select-class': selecionarTurma(target.dataset.turma || '', target); break;
    case 'select-bread': selectBread(value); break;
    case 'toggle-complement': toggleComp(target); break;
    case 'toggle-sauce': toggleSauce(target); break;
    case 'select-juice': selectJuice(target); break;
    case 'add-soda': addSoda(target.dataset.soda || ''); break;
    case 'remove-soda': removeSoda(Number.parseInt(target.dataset.index || '-1', 10)); break;
    case 'add-sweet': addSweet(target.dataset.sweet || ''); break;
    case 'remove-sweet': removeSweet(target.dataset.sweet || ''); break;
    case 'select-payment': selectPayment(value); break;
    case 'submit-order': await submitOrder(); break;
    case 'back-home': voltarInicio(); break;
    case 'refresh-admin': refreshAdmin(); break;
    case 'sync-locks': await syncUserOrderLocks(); break;
    case 'filter-orders': filterOrders(value, target); break;
    case 'filter-class': filterByTurma(target.dataset.value || 'todas'); break;
    case 'confirm-payment': await confirmPayment(target.dataset.orderId || ''); break;
    case 'toggle-hidden': await toggleOculto(target.dataset.orderId || ''); break;
    case 'open-cancel': openCancelModal(target.dataset.orderId || ''); break;
    case 'close-cancel': closeCancelModal(); break;
    case 'confirm-cancel': await confirmCancelOrder(); break;
    default: break;
  }
}

document.addEventListener('click', event => {
  handleActionClick(event).catch(err => console.error('Falha ao executar ação:', err));
});

document.getElementById('customer-name')?.addEventListener('input', onNomeInput);
document.getElementById('profile-name-input')?.addEventListener('input', updateProfileSaveState);
document.getElementById('profile-turma-select')?.addEventListener('change', updateProfileSaveState);
document.getElementById('admin-turma-filter')?.addEventListener('change', event => {
  filterByTurma(event.target.value);
});
