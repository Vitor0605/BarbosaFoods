...
import {
  getAuth, GoogleAuthProvider, signInWithPopup, signInWithRedirect, getRedirectResult, signOut, onAuthStateChanged, browserLocalPersistence, browserSessionPersistence, inMemoryPersistence, setPersistence
...
function openProfile() {
  ...
  if (nameInput) nameInput.oninput = updateProfileSaveState;
  const turmaSelect = document.getElementById('profile-turma-select');
  if (turmaSelect) turmaSelect.onchange = updateProfileSaveState;
...
function explainAuthError(err) {
  ...
  if (code === 'auth/popup-blocked') return 'O navegador bloqueou a janela do Google. Vou tentar abrir o login em tela inteira.';
  ...
  if (code === 'auth/web-storage-unsupported') return 'O navegador bloqueou o armazenamento necessário para manter o login.';
  return (err && err.message) ? err.message : 'Não foi possível concluir o login.';
}

async function ensureAuthPersistence() {
  const options = [browserLocalPersistence, browserSessionPersistence, inMemoryPersistence];
  let lastError = null;
...
async function loginWithGoogle() {
  ...
  try {
    ...
    if (!isMobileBrowser() && fallbackCodes.has(err && err.code)) {
      showAuthMessage('Popup bloqueado. Abrindo login em tela inteira...', 'info');
      await signInWithRedirect(auth, provider);
      return;
    }
...
Object.assign(window, {
  ...
  openProfile, closeProfile, saveProfileFromModal, updateProfileSaveState, cancelMyOrder,
  openCancelModal, closeCancelModal, confirmCancelOrder, openLogoutModal, closeLogoutModal, confirmLogout, startNewOrder
});
