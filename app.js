...
import {
  ...
  onSnapshot, query, orderBy, where, getDoc, setDoc, getDocs, runTransaction, writeBatch, enableIndexedDbPersistence
...
// Ajuda em celulares com internet instável: o Firestore mantém cache local
// e evita que pequenas quedas de conexão derrubem a experiência inteira.
enableIndexedDbPersistence(db).catch(err => {
  console.warn('Persistência local do Firestore não ativada:', err && err.code ? err.code : err);
});
...
function friendlyAuthError(err) {
  ...
  if (code === 'auth/unauthorized-domain') {
    return 'Este domínio ainda não está autorizado no Firebase. Adicione o domínio do seu site em Firebase Authentication > Settings > Authorized domains.';
  }
  if (code === 'auth/popup-closed-by-user') return 'Login cancelado antes de concluir.';
  if (code === 'auth/popup-blocked') return 'O navegador bloqueou a janela de login. Vou tentar abrir por redirecionamento.';
  if (code === 'auth/network-request-failed') return 'Falha de rede no login. Verifique a internet, desative VPN/adblock se estiver usando e tente novamente.';
  if (code === 'auth/operation-not-supported-in-this-environment') return 'Este navegador não aceitou o popup do Google. Vou tentar pelo redirecionamento.';
  return err && err.message ? `Erro no login: ${err.message}` : 'Erro ao fazer login com o Google.';
}

function friendlyFirestoreError(err) {
  const code = err && err.code ? err.code : '';
  if (code === 'permission-denied') {
    return 'O banco de dados bloqueou o salvamento. Verifique as regras do Firestore para permitir que o usuário logado crie pedidos com o próprio UID.';
  }
  if (code === 'unavailable' || code === 'deadline-exceeded' || code === 'aborted') {
    return 'A conexão com o banco oscilou. Tente novamente em alguns segundos.';
  }
  if (code === 'failed-precondition') {
    return 'O banco pediu uma configuração extra. Verifique o console do Firebase/Vercel para ver se falta índice ou regra.';
  }
  return err && err.message ? err.message : 'Erro ao salvar o pedido.';
}

function isTransientFirestoreError(err) {
  const code = err && err.code ? err.code : '';
  return ['unavailable', 'deadline-exceeded', 'aborted', 'cancelled', 'internal'].includes(code);
}
...
async function retryFirestoreWrite(action, onRetry) {
  let lastError = null;
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      ...
      lastError = err;
      if (!isTransientFirestoreError(err) || attempt === 3) break;
...
const MAX_SODAS       = 3;
const MAX_SWEETS      = 10;
const MAX_ORDERS_PER_USER = 3;       // limite total de pedidos válidos (não cancelados) por conta
...
function countValidOrders(orders) {
  // Pedidos cancelados não contam mais no limite total da conta.
...
function analyzeOrderPermission(orders) {
  ...
  if (todayOrders.length >= MAX_ORDERS_PER_DAY) {
    return {
      podePedir: false,
      motivo: `Você já fez ${MAX_ORDERS_PER_DAY} pedidos hoje. Esse limite diário conta pedidos pagos, pendentes e cancelados.`,
      pedidos: userOrders,
      validOrders,
      todayOrders,
      openOrder: null,
  ...
  if (validOrders.length >= MAX_ORDERS_PER_USER) {
    return {
      podePedir: false,
      motivo: `Você já atingiu o limite de ${MAX_ORDERS_PER_USER} pedidos válidos nesta conta. Pedidos cancelados não entram nessa contagem.`,
      pedidos: userOrders,
      validOrders,
      todayOrders,
      openOrder: null,
...
function renderProfileOrders() {
  ...
  renderOrderCards(list, allOrders.filter(o => o.userUid === currentUser.uid));
}
...
function renderStudentHome() {
  ...
  const validCount = status.validOrders ? status.validOrders.length : countValidOrders(meusPedidos);
  const todayCount = status.todayOrders ? status.todayOrders.length : getTodayOrders(meusPedidos).length;
  ...
  if (countBox) {
    countBox.textContent = `${validCount}/${MAX_ORDERS_PER_USER} pedidos válidos · ${todayCount}/${MAX_ORDERS_PER_DAY} pedidos hoje`;
  }
  ...
  if (alertBox) {
    if (status.podePedir) {
      ...
      alertBox.textContent = meusPedidos.length ? 'Você está liberado para fazer outro pedido.' : 'Você ainda não fez pedidos. Pode começar agora.';
    } else {
      alertBox.style.display = 'block';
  ...
  renderOrderCards(ordersList, meusPedidos);
...
function openCancelModal(fid) {
  ...
  if (text) text.textContent = 'Esse pedido será marcado como cancelado. Ele não vai contar no limite total de 3 pedidos válidos, mas continua contando no limite diário.';
  if (preview) {
    preview.innerHTML = `
...
function onNomeInput() {
  ...
  if (!st.podePedir) {
    ...
    const validCount = st.validOrders ? st.validOrders.length : countValidOrders(st.pedidos);
    const todayCount = st.todayOrders ? st.todayOrders.length : getTodayOrders(st.pedidos).length;
    statusEl.innerHTML  = `<div class="student-status-card ok">✅ Liberado. ${validCount}/${MAX_ORDERS_PER_USER} pedidos válidos · ${todayCount}/${MAX_ORDERS_PER_DAY} pedidos hoje.</div>`;
...
async function submitOrder() {
  ...
  // Consulta fresca antes de salvar. Se a internet oscilar no celular,
  // usamos o estado local em vez de travar o pedido com falso "erro de conexão".
  let freshOrders = [];
  try {
    freshOrders = await fetchMyOrdersOnce();
  } catch (err) {
    console.warn('Não foi possível validar pedidos no servidor. Usando estado local:', err);
    freshOrders = pedidosDoAluno(nome, turma);
  }
  ...
  try {
    ...
    await retryFirestoreWrite(
      async () => setDoc(newOrderRef, order),
      attempt => { btn.textContent = `Conexão oscilou... tentando ${attempt}/3`; }
    );
    ...
    const profilePatch = {
      ...
      pedidosValidosCount: (serverStatus.validOrders || []).length + 1,
      pedidosHojeCount: (serverStatus.todayOrders || []).length + 1,
      updatedAt: now.toISOString()
    };

    // Atualizar o perfil ajuda nas travas, mas não deve apagar o pedido caso
    // a regra do documento /usuarios esteja mais restrita que /pedidos.
    setDoc(userRef, profilePatch, { merge: true })
      .then(() => { currentProfile = { ...(currentProfile || {}), ...profilePatch }; })
      .catch(err => console.warn('Pedido salvo, mas não foi possível atualizar o perfil:', err));

    allOrders = [
      { firestoreId: newOrderRef.id, ...order },
      ...allOrders.filter(o => o.firestoreId !== newOrderRef.id)
    ];
    renderProfileOrders();
    renderStudentHome();
    ...
    const isBusinessRule = msg.includes('Limite') || msg.includes('aguardando') || msg.includes('bloqueado') || msg.includes('atingiu');
    showOrderError(isBusinessRule ? msg : friendlyFirestoreError(err));
  } finally {
    btn.disabled    = false;
...
getRedirectResult(auth)
  .then(result => {
    sessionStorage.removeItem('bf_login_redirect_pending');
...
async function loginWithGoogle() {
  ...
  try {
    ...
    // Primeiro tenta popup também no celular. Em vários navegadores mobile,
    // isso evita o efeito de "a página reiniciou e não logou" do redirect.
    await signInWithPopup(auth, provider);
  } catch (err) {
    const code = err && err.code ? err.code : '';

    if (code === 'auth/popup-closed-by-user') {
      showAuthMessage('Login cancelado. Toque novamente em Entrar com o Google para tentar de novo.', 'warn');
      return;
    }

    if (code !== 'auth/unauthorized-domain' && (redirectFallbackCodes.has(code) || isMobileBrowser())) {
      try {
        showAuthMessage('Abrindo login em outra página... depois você volta automaticamente para o BarbosaFoods.', 'info');
        sessionStorage.setItem('bf_login_redirect_pending', '1');
        await signInWithRedirect(auth, provider);
        return;
      } catch (redirectErr) {
        sessionStorage.removeItem('bf_login_redirect_pending');
        console.error('Erro no redirect do login:', redirectErr);
        showAuthMessage(friendlyAuthError(redirectErr), 'error');
        return;
      }
    }
